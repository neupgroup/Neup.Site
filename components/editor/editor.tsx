...
const Editor: FC<EditorProps> = ({ initialElements, pageId: initialPageId, pageNotFound }) => {
            ...
            setElements(prevElements => {
                ...
                const addElementToParent = (els: CanvasElementData[], pId: string, element: CanvasElementData): boolean => {
                    for (let i = 0; i < els.length; i++) {
                        ...
                        if (els[i].children && addElementToParent(els[i].children as CanvasElementData[], pId, element)) {
                            return true;
                        }
                    }
                    return false;
                    ...
                    const addSibling = (els: CanvasElementData[], targetId: string, parentId?: string): CanvasElementData[] => {
                        if (parentId) {
                            for (let i = 0; i < els.length; i++) {
                                ...
                                if (els[i].children) {
                                    addSibling(els[i].children as CanvasElementData[], targetId, parentId);
                                }
                            }
                        } else {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            try {
                ...
                logErrorToDatabase({ message: (error as Error).message, stack: (error as Error).stack });
            }
        };
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            logErrorToDatabase({ message: error.message, stack: error.stack });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="flex h-screen w-full flex-col bg-background text-foreground" onDragLeave={handleDragLeave}>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                />
                <main ref={canvasRef} className={cn("flex-1 overflow-y-auto bg-background custom-scrollbar relative transition-all duration-300", isNavigating && "filter blur-sm pointer-events-none")}>
                    {pageNotFound ? (
                        <div className="flex flex-col items-center justify-center h-full text-center p-4">
                            <h2 className="text-2xl font-bold mb-2">Page Not Found</h2>
                            <p className="text-muted-foreground mb-4">The page you are looking for does not exist or has been deleted.</p>
                            <p className="text-sm text-muted-foreground">Select another page from the sidebar or create a new one.</p>
                        </div>
                    ) : (
                        <>
                            <Canvas
