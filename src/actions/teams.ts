...
export async function createTeam(data: Omit<Team, 'id'>): Promise<{ success: boolean; id?: string; error?: string }> {
  try {
    ...
    await logErrorToFirestore({ message: `Failed to create team: ${e.message}`, stack: e.stack, source: 'createTeam' });
...
export async function getTeams(): Promise<{ success: boolean; teams?: Team[]; error?: string }> {
  try {
    ...
    await logErrorToFirestore({ message: `Failed to get teams: ${e.message}`, stack: e.stack, source: 'getTeams' });
...
export async function getTeam(id: string): Promise<{ success: boolean; team?: Team; error?: string }> {
    try {
        ...
        await logErrorToFirestore({ message: `Failed to get team ${id}: ${e.message}`, stack: e.stack, source: 'getTeam' });
...
export async function updateTeam(id: string, data: Partial<Omit<Team, 'id'>>): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    await logErrorToFirestore({ message: `Failed to update team ${id}: ${e.message}`, stack: e.stack, source: 'updateTeam' });
...
export async function deleteTeam(id: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    await logErrorToFirestore({ message: `Failed to delete team ${id}: ${e.message}`, stack: e.stack, source: 'deleteTeam' });
